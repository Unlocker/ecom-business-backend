...
package object types {
  ...
  abstract class RichNewtype[A: JsonCodec : Schema : Equal : Ord] extends Newtype[A] {
    self =>
  ...
  abstract class RichSubtype[A: JsonCodec : Schema : Ord] extends Subtype[A] {
    self =>
    implicit val equiv: A <=> Type = Equivalence(wrap, unwrap)
    implicit val equal: Equal[Type] = implicitly[Equal[A]].contramap(unwrap)
    implicit val jsonCodec: JsonCodec[Type] = implicitly[JsonCodec[A]].transform(wrap, unwrap)
  ...
  implicit val currencyEncoder: JsonEncoder[Currency] = JsonEncoder[String].contramap(_.getCurrencyCode)
  implicit val currencyDecoder: JsonDecoder[Currency] = JsonDecoder[String].map(Currency.getInstance)
