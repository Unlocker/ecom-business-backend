...
object SchemeConverter {
	...
	implicit class ZioSchema [E, A](data: ZIO[Any, E, A]) {
		...
		def asModelWithMapError[E2 <: E, B](fe: E => E2)(implicit f: A => B): ZIO[Any, E2, B] = data.mapBoth(fe, f)
	}
	...
	implicit class ZioSchemaF [F[+_] : ForEach, E, A](data: ZIO[Any, E, F[A]]) {
		...
		def asModelWithMapError[E2 <: E, B](fe: E => E2)(implicit f: A => B): ZIO[Any, E2, F[B]] = data.mapBoth(fe, el => ForEach[F].map(f)(el))
	}
	...
	implicit class ZioSchemaFF [F[+_] : ForEach : AssociativeFlatten, E, A](data: ZIO[Any, E, F[F[A]]]) {
		...
		def asModelWithMapError[E2 <: E, B](fe: E => E2)(implicit f: A => B): ZIO[Any, E2, F[B]] = data.mapBoth(fe, el => ForEach[F].map(f)(AssociativeFlatten[F].flatten(el)))
	}
